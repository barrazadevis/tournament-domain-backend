...
Las 3 migraciones, el dominio, el puerto+adaptador de Piston, los 2 casos de
uso, el wiring HTTP/socket y las 3 pantallas de frontend descritas abajo ya
están implementados. 95 tests de backend en verde, verificación end-to-end
en navegador contra un Piston auto-hospedado local (ver sección "Cómo correr
Piston localmente" al final).

**Hallazgo real durante la verificación**: la API pública de Piston
(`emkc.org`) pasó a ser whitelist-only el 15/02/2026 — deja de servir para
uso real sin pedir acceso. El manejo de error (`status: 'ERROR'`,
degradación sin romper la submission) funcionó exactamente como se diseñó
cuando esto pasó. Para probar/usar la ejecución real hace falta
auto-hospedar Piston (ver abajo) — la API pública ya no es una opción
viable ni siquiera para desarrollo.
...
`SqliteTournamentRepository`'s `upsertBusinessCaseRow` (`ON CONFLICT DO
NOTHING`, se ejecuta en cada save del agregado pero es no-op tras la
...
- **`BusinessCase`**: nuevo campo `testCases: {input: string;
  expectedOutput: string}[]`, readonly, solo por constructor — la entidad
...
- `src/application/ports/code-runner.ts`: `CodeRunner.run({language, code,
  stdin}): Promise<{stdout, stderr, exitCode, timedOut}>` — nunca lanza;
...
- `src/infrastructure/piston/piston-code-runner.ts`: `fetch` nativo
  (Node 24, sin dependencia nueva) a `POST {PISTON_URL ??
  'https://emkc.org/api/v2/piston'}/api/v2/execute`, con
...
Ambos casos de uso se saltan por completo si `tournament.getLanguage() !==
'PYTHON'` — cero cambio de comportamiento para torneos PSeInt.

### 6. `StartTournamentUseCase`

- `StartTournamentInput` gana `language: 'PYTHON' | 'PSEINT'` (top-level,
  todo el torneo).
- Cada `StartTournamentCaseInput` gana `testCases?: {input,
  expectedOutput}[]`.
...
- `MatchesController.getById()` (usada por `TeamMatchPage`): agregar
  ...
  **resumen**, no el detalle completo:
  ...
  patrón).
- `TournamentPresenter.presentSubmission()` (usada por `GET
  /tournaments/:id`, que alimenta `JudgePage`): esta SÍ ya expone
...
## Cómo correr Piston localmente (necesario — la API pública ya no sirve)

```bash
# 1. Docker Desktop debe estar corriendo.
# 2. Crear un volumen (evita problemas de bind-mount de rutas Windows):
docker volume create piston_data

# 3. Levantar el contenedor — --privileged es requerido por Piston mismo
#    (su sandboxing interno vía Isolate necesita capacidades de kernel):
docker run --privileged -p 127.0.0.1:2000:2000 -v piston_data:/piston -dit --name piston_api ghcr.io/engineer-man/piston

# 4. Instalar el paquete de Python (el contenedor arranca sin lenguajes):
curl -X POST http://localhost:2000/api/v2/packages -H "Content-Type: application/json" -d "{\"language\":\"python\",\"version\":\"3.10.0\"}"

# 5. Arrancar el backend apuntando ahí. OJO: el valor debe incluir /api/v2,
#    no solo el host (ver comentario en piston-code-runner.ts):
PISTON_URL=http://localhost:2000/api/v2 npm run dev
```

Para reiniciar en una sesión futura (el contenedor ya existe, solo hay que
prenderlo): `docker start piston_api`, luego arrancar el backend con el
mismo `PISTON_URL`.
